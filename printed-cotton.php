...
$sql_select_data = "select * from `product` where `category`='Printed Cotton' AND `stock`='In Stock'";
...
$sql_select = "select * from `product` where `category`='Printed Cotton' AND `stock`='In Stock' limit $start,$limit";
...
if (isset($_GET['search_acc_product']))
{
	...
	$sql_select_data = "select * from `product` where `category`='Printed Cotton' AND `stock`='In Stock' AND (`name` like '%$search_pro%' OR `category` like '%$search_pro%' OR `tag` like '%$search_pro%' OR `type` like '%$search_pro%' OR `one_line_title` like '%$search_pro%' OR `size` like '%$search_pro%' OR `color` like '%$search_pro%' OR `description` like '%$search_pro%' OR `weight` like '%$search_pro%' OR `dimension` like '%$search_pro%' OR `material` like '%$search_pro%')";
	$data_data = mysqli_query($conn,$sql_select_data);
	$data_count = mysqli_num_rows($data_data);
	...
	$sql_select = "select * from `product` where `category`='Printed Cotton' AND `stock`='In Stock' AND (`name` like '%$search_pro%' OR `category` like '%$search_pro%' OR `tag` like '%$search_pro%' OR `type` like '%$search_pro%' OR `one_line_title` like '%$search_pro%' OR `size` like '%$search_pro%' OR `color` like '%$search_pro%' OR `description` like '%$search_pro%' OR `weight` like '%$search_pro%' OR `dimension` like '%$search_pro%' OR `material` like '%$search_pro%') limit $start,$limit";
	$data = mysqli_query($conn,$sql_select);
	...
	<div class="p-b-140" style="background: #fff;">
		<div class="container">
			<div class="text-center p-t-60 p-b-40 premium-animate">
				<h1 style="font-family: 'Poppins', sans-serif; font-size: 36px; font-weight: 800; color: #111; letter-spacing: -0.5px;">Printed Cotton Hub</h1>
				<p style="color: #888; font-size: 14px;">Vibrant prints, superior cotton quality</p>
			</div>
			<div class="flex-w flex-sb-m p-b-52 premium-animate">
				<div class="category-nav-wrapper">
					...
					<a href="printed-cotton.php" class="category-nav-link active">Printed Cotton</a>
				...
				<div class="dis-none panel-search w-full p-t-10 p-b-15">
					...
					<form method="get">
						<input class="mtext-107 cl2 size-114 plh2 p-r-15" type="text" name="search_acc_product" placeholder="Search Printed Cotton">
					</form>
					</div>	
				</div>
			</div>
