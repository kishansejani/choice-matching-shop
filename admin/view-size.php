...
if (isset($_GET['d_id'])) {
    $id = $_GET['d_id'];
    mysqli_query($conn, "DELETE FROM `size` WHERE `id`='$id'");
    header('location:view-size.php?success=1');
    exit;
}
...
if (isset($_GET['toggle_status'])) {
    ...
    mysqli_query($conn, "UPDATE `size` SET `status`='$status' WHERE `id`='$id'");
            ...
            <div class="card">
                <div class="card-body">
                    <table id="example2" class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
                            <?php
                            $res = mysqli_query($conn, "SELECT * FROM `size` ORDER BY sequence ASC, id DESC");
                            while ($row = mysqli_fetch_assoc($res)) {
                                ...
                                echo "<tr>
                                    ...
                                    <td>
                                        <a href='edit-size.php?e_id={$row['id']}' class='btn btn-primary'>Edit</a>
                                    </td>
                                    <td>
