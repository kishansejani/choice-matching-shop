		...
		$(document).ready(function(){
			...
			function updateWishlistIcons() {
				...
				$('.js-addwish-b2, .js-addwish-detail').each(function(){
					let id = $(this).data('id') || $(this).attr('attr_id');
			...
			$(document).on('click', '.js-addwish-b2, .js-addwish-detail', function(e){
				...
				const button = $(this);
				const parent = button.closest('.block2, .p-t-33, .row');
				const product = {
					id: button.data('id') || button.attr('attr_id'),
					name: button.data('name') || parent.find('.js-name-b2, .js-name-detail').text().trim(),
					price: button.data('price') || parent.find('.stext-105, .mtext-106, .cl3').text().replace('Rs.', '').replace('$', '').trim(),
					image: button.data('image') || parent.find('img').attr('src')
				};

				if (!product.id) {
					product.id = btoa(product.name); 
				}

				if (addToWishlist(product)) {
					swal(product.name, "is added to your wishlist !", "success");
